...
import { generateNarrationScript } from "../narration/services/script-generator.service.js";
import {
  assertNarrationLlmTopology,
  publishNarrationGenerationResponse,
} from "../infrastructure/rabbitmq.js";
import {
  narrationGenerationRequestSchema,
  type NarrationGenerationRequest,
  type NarrationGenerationResponse,
} from "../types/narrationGeneration.js";
import { createScopedLogger } from "../utils/logger.js";

const consumerLogger = createScopedLogger("consumer.narrationGeneration");
...
type ParsedMessageOutcome =
  | { kind: "valid"; request: NarrationGenerationRequest }
...
const parseAndValidateMessage = (
  ...
  const validationResult =
    narrationGenerationRequestSchema.safeParse(parsedJsonPayload);
...
const buildResponse = async (
  request: NarrationGenerationRequest,
): Promise<NarrationGenerationResponse> => {
  try {
    const script = await generateNarrationScript(request.context);
    ...
    if (script.lines.length === 0) {
      return {
        ...
        status: "failed",
        errorMessage: "Narration generation returned no lines",
      };
    }
...
export const startNarrationGenerationConsumer = async (): Promise<void> => {
  const channel = await assertNarrationLlmTopology();
  await channel.prefetch(env.NARRATION_LLM_PREFETCH);

  consumerLogger.info("Consumer starting", {
    queue: env.NARRATION_LLM_REQUEST_QUEUE,
    prefetch: env.NARRATION_LLM_PREFETCH,
    maxRetries: env.NARRATION_LLM_MAX_RETRIES,
  });
  ...
  await channel.consume(
    ...
    async (consumeMessage) => {
      ...
      try {
        ...
        await publishNarrationGenerationResponse(response);

        consumerLogger.info("Narration generation response published", {
          videoId: parsed.request.videoId,
          jobId: parsed.request.jobId,
          status: response.status,
          lineCount: response.script?.lines.length ?? 0,
        });
        ...
        if (previousAttempts >= env.NARRATION_LLM_MAX_RETRIES) {
          try {
            const reparsed = parseAndValidateMessage(consumeMessage);
            if (reparsed.kind === "valid") {
              await publishNarrationGenerationResponse({
        ...
        consumerLogger.warn("Retrying narration generation message", {
          retryKey,

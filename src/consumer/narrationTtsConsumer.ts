...
import {
  assertNarrationTtsTopology,
  publishNarrationTtsResponse,
} from "../infrastructure/rabbitmq.js";
import { synthesizeSpeechToMp3 } from "../tts/tts.service.js";
import {
  narrationTtsRequestSchema,
  type NarrationTtsRequest,
  type NarrationTtsResponse,
} from "../types/narrationTts.js";
import { createScopedLogger } from "../utils/logger.js";

const consumerLogger = createScopedLogger("consumer.narrationTts");
...
type ParsedMessageOutcome =
  | { kind: "valid"; request: NarrationTtsRequest }
...
const parseAndValidateMessage = (
  ...
  const validationResult = narrationTtsRequestSchema.safeParse(parsedJsonPayload);
...
const buildResponse = async (
  request: NarrationTtsRequest,
): Promise<NarrationTtsResponse> => {
  try {
    const audioBuffer = await synthesizeSpeechToMp3(request.narrationScript, {
      jobId: request.jobId,
...
export const startNarrationTtsConsumer = async (): Promise<void> => {
  const channel = await assertNarrationTtsTopology();
  await channel.prefetch(env.NARRATION_TTS_PREFETCH);

  consumerLogger.info("Consumer starting", {
    queue: env.NARRATION_TTS_REQUEST_QUEUE,
    prefetch: env.NARRATION_TTS_PREFETCH,
    maxRetries: env.NARRATION_TTS_MAX_RETRIES,
  });
  ...
  await channel.consume(
    ...
    async (consumeMessage) => {
      ...
      try {
        ...
        await publishNarrationTtsResponse(response);

        consumerLogger.info("Narration TTS response published", {
          videoId: parsed.request.videoId,
          jobId: parsed.request.jobId,
          status: response.status,
          audioBytes:
            response.audioBase64 !== undefined
              ? Math.ceil((response.audioBase64.length * 3) / 4)
              : 0,
        });
        ...
        if (previousAttempts >= env.NARRATION_TTS_MAX_RETRIES) {
          try {
            const reparsed = parseAndValidateMessage(consumeMessage);
            if (reparsed.kind === "valid") {
              await publishNarrationTtsResponse({
        ...
        consumerLogger.warn("Retrying narration TTS message", {
          retryKey,
